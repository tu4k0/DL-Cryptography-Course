...
def main():
    ...
    print(f"Alice:\t({SHA1_own(str(Alice.public_key.x))}, {SHA1_own(str(Alice.public_key.y))})")
    print(f"Bob:\t({SHA1_own(str(Bob.public_key.x))}, {SHA1_own(str(Bob.public_key.y))})")
    ...
    ec = ElipticCurve()
    Alice.shared_secret = ec.scalar_EC_point(Alice.private_key.private_numbers().private_value, Alice)
    Bob.shared_secret = ec.scalar_EC_point(Bob.private_key.private_numbers().private_value, Bob)
